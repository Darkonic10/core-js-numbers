...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multiplyXY = x1 * x2 + y1 * y2;
  const vector1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vector2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(multiplyXY / (vector1 * vector2));
}
...
function isPowerOfTwo(num) {
  const powerOfTwo = Math.log2(num);
  return powerOfTwo % 1 === 0;
}
...
function getCountOfOddNumbers(number) {
  return number < 0 ? Math.abs(Math.floor(number / 2)) : Math.ceil(number / 2);
}
